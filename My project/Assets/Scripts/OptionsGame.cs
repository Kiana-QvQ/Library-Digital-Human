...
#if UNITY_POSTPROCESSING_STACK_V2
...
public class OptionsGame : MonoBehaviour
{
	...
	private void Awake()
	{
		// 如果已经存在一个全局的 gameOptions（来自上一个场景），
		// 优先使用当前场景中的这个实例：先销毁旧的，避免旧场景的 Game/MenuGame/Mita 持续保留在 DontDestroyOnLoad 里。
		if (GlobalTag.gameOptions != null && GlobalTag.gameOptions != base.gameObject)
		{
			Object.DestroyImmediate(GlobalTag.gameOptions);
			GlobalTag.gameOptions = null;
			GlobalGame.optionGameLoad = false;
		}
		...
		if (!GlobalGame.optionGameLoad)
		{
			...
			GlobalGame.Language = PlayerPrefs.GetString("Language", "Chinese");
			// 只保留中文：无论历史存档写了什么语言，都强制使用 Chinese
			if (GlobalGame.Language != "Chinese")
			{
				GlobalGame.Language = "Chinese";
			...
			AchievementsController achievements = GetComponent<AchievementsController>();
			if (achievements != null)
				achievements.StartComponent();
			else
				Debug.LogWarning("OptionsGame: AchievementsController component not found on this GameObject.");
	...
	{
		string text = Path.Combine(Application.streamingAssetsPath, "Data", "Languages", GlobalGame.Language);
		...
		for (int i = 0; i < files.Length; i++)
		{
			...
			list[i].name = files[i];
			list[i].strings = File.ReadAllLines(Path.Combine(text, files[i] + ".txt"));
		...
		// 中文优先：避免强制替换为不含中文字形的字体，保持 UI 上原本配置的字体
		if (GlobalGame.Language == "Chinese")
		{
			GlobalGame.fontUse = null;
			return;
		}
