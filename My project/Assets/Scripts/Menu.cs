...
public class Menu : MonoBehaviour
{
	...
	private void Start()
	{
		...
		// 处理成就进度显示（在缺少 UI 或 gameOptions 时也不报错）
		if (textAchievementProgress != null)
		{
			if (GlobalTag.gameOptions != null)
			{
				AchievementsController achievements = GlobalTag.gameOptions.GetComponent<AchievementsController>();
				if (achievements != null)
				{
					textAchievementProgress.text = achievements.ProgressAchievement() + "%";
				}
				else
				{
					textAchievementProgress.text = "0%";
				}
			}
			else
			{
		...
		{
			MenuLocation menuLocation = firstLocation.GetComponent<MenuLocation>();
			if (menuLocation != null)
			{
				menuLocation.Active(x: true);
			}
		}
