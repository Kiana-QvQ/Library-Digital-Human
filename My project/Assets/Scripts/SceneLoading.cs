...
public class SceneLoading : MonoBehaviour
{
	...
	public void FirstStartGame()
	{
		...
		string[] directories = Directory.GetDirectories(languagesRoot);
		for (int i = 0; i < directories.Length; i++)
		{
			directories[i] = directories[i].Replace(languagesRoot + Path.DirectorySeparatorChar, "");
		}
		for (int j = 0; j < directories.Length; j++)
		{
			...
			texture2D.LoadImage(File.ReadAllBytes(Path.Combine(languagesRoot, directories[j], "F.png")));
