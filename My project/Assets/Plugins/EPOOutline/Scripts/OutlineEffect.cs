...
namespace EPOOutline
{
    public static class OutlineEffect
    {
        ...
        /// <summary>
        /// Shader 在 Resources 中加载失败时，用 Shader.Find 的回退名称（打包时可能被剥离，需在 Graphics - Always Included Shaders 中加入）。
        /// </summary>
        private static string GetFallbackShaderName(string resourceName)
        {
            if (resourceName == "Fills/FillMask") return "Hidden/EPO/Fill/Utils/Fill mask";
            return "Hidden/" + resourceName.Replace("Fills/", "");
        }

        private static Material LoadMaterial(string shaderName)
        {
            var resourcePath = string.Format("Easy performant outline/Shaders/{0}", shaderName);
            var shader = Resources.Load<Shader>(resourcePath);
            if (shader == null)
                shader = Shader.Find(GetFallbackShaderName(shaderName));
            if (shader == null)
            {
                Debug.LogError($"[EPOOutline] Shader not found: '{resourcePath}' and Fallback '{GetFallbackShaderName(shaderName)}'. " +
                    "Add EPO Outline shaders to Edit > Project Settings > Graphics > Always Included Shaders, and ensure 'Assets/Plugins/EPOOutline/Resources' is in the build.");
                return null;
            }
        ...
        private static void InitMaterials()
        {
            ...
            if (ObstacleMaterial == null || OutlineMaterial == null || ZPrepassMaterial == null || DilateMaterial == null
                || BlurMaterial == null || FinalBlitMaterial == null || BasicBlitMaterial == null
                || FillMaskMaterial == null || ClearStencilMaterial == null)
            {
                throw new InvalidOperationException(
                    "EPOOutline: One or more shaders failed to load. Fix: Edit > Project Settings > Graphics > " +
                    "Always Included Shaders — add all shaders from 'Assets/Plugins/EPOOutline/Resources/Easy performant outline/Shaders' (e.g. Obstacle, Outline, ZPrepass, Dilate, Blur, FinalBlit, BasicBlit, ClearStencil, Fills/FillMask). " +
