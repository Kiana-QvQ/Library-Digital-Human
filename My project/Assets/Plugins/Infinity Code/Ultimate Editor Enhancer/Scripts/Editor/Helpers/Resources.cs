    ...
    public static class Resources
    {
            ...
            get
            {
                if (string.IsNullOrEmpty(_assetFolder))
                {
                    ...
                    if (paths.Length != 0)
                    {
                        ...
                        string rawPath = info.Directory.Parent.FullName.Substring(Application.dataPath.Length - 6);
                        _assetFolder = rawPath.Replace('\\', '/') + "/";
                    }
                    else
                    {
        ...
        public static string settingsFolder
        {
            get
            {
                ...
                {
                    // Build from assetFolder using only forward slashes (Unity asset paths)
                    string folder = assetFolder.TrimEnd('/');
                    int lastSlash = folder.LastIndexOf('/');
                    _settingsFolder = lastSlash >= 0
                        ? folder.Substring(0, lastSlash + 1) + "Ultimate Editor Enhancer Settings/"
                        : "Assets/Plugins/Infinity Code/Ultimate Editor Enhancer Settings/";
                }
