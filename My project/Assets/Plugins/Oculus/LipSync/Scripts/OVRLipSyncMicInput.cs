...
public class OVRLipSyncMicInput : MonoBehaviour
{
    ...
    private static extern void WebGLStartMicrophone(int frequency);

    [DllImport("__Internal")]
    private static extern void WebGLStopMicrophone();

    [DllImport("__Internal")]
    private static extern bool WebGLIsRecording();
#endif
    ...
    {
#if UNITY_WEBGL && !UNITY_EDITOR
        WebGLStartMicrophone(micFrequency);
#else
        if (string.IsNullOrEmpty(selectedDevice)) return;
