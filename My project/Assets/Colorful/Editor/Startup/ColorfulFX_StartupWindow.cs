...
public sealed class ColorfulFX_StartupWindow : EditorWindow
{
    ...
    {
        // Get the relative data path (use forward slashes for Unity paths on all platforms)
    ...
    {
        TextAsset textAsset = LoadAssetAt<TextAsset>(pathChangelog);
        if (textAsset == null || string.IsNullOrEmpty(textAsset.text)) return "";
    ...
    void OnEnable()
    {
        ...
        if (!string.IsNullOrEmpty(version)) EditorPrefs.SetString(identifier, version);

        TextAsset changelog = LoadAssetAt<TextAsset>(pathChangelog);
        changelogText = changelog != null ? changelog.text : "";
