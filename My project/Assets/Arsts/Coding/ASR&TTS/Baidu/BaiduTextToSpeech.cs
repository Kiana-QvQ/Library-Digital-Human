...
public class BaiduTextToSpeech : TTS
{
    ...
    {
        StartCoroutine(RefreshTokenAndSpeak(_msg, _callback));
    }

    private IEnumerator RefreshTokenAndSpeak(string _msg, Action<AudioClip, string> _callback)
    {
        yield return m_Settings.RefreshToken();
        yield return GetSpeech(_msg, _callback);
    }
