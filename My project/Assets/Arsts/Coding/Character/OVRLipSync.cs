...
public class OVRLipSync : MonoBehaviour
{
    ...
    public static Result Initialize()
    {
        ...
        // Get the current sample rate and buffer size
        // Note: AudioSettings.GetConfiguration() may not be available in all Unity versions
        // Using reflection as a fallback for compatibility
        try
        {
            // Try to get AudioConfiguration via reflection (for Unity versions that support it)
            System.Reflection.MethodInfo getConfigMethod = typeof(AudioSettings).GetMethod("GetConfiguration", 
                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
            
            if (getConfigMethod != null)
            {
                object audioConfig = getConfigMethod.Invoke(null, null);
                System.Type configType = audioConfig.GetType();
                sampleRate = (int)configType.GetProperty("sampleRate").GetValue(audioConfig);
                bufferSize = (int)configType.GetProperty("dspBufferSize").GetValue(audioConfig);
                numbuf = (int)configType.GetProperty("numRealVoices").GetValue(audioConfig);
            }
            else
            {
                // Fallback: Use default/common audio settings
                sampleRate = 48000;  // Common sample rate
                bufferSize = 1024;   // Common buffer size
                numbuf = 32;         // Common voice count
                Debug.LogWarning("AudioSettings.GetConfiguration() not available, using default values.");
            }
        }
        catch (System.Exception e)
        {
            Debug.LogWarning($"Failed to get audio configuration: {e.Message}. Using default values.");
            sampleRate = 48000;
            bufferSize = 1024;
            numbuf = 32;
        }
